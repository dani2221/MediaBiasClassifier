...
const genUrls = () => {
    const urls = [];
    for(let i=1;i<=500;i++){
        urls.push(`https://sdk.mk/index.php/category/makedonija/page/${i}`);
    }
    return urls;
}
const getData = async () => {
    ...
    for(let url of genUrls()){
        ...
        let items = []; 
        for(let child of dom.window.document.getElementsByClassName('item-title')){
